...
- `import` without `--force` takes the file's row metadata — alias, icon, held, preferred — for an account it already holds even when the credential is kept, reported as `updated` (2026-09-11, parity day 6, #20): the exporter is where the user labels accounts in phase 1, and a label changed after the first import never reached swapd (the active account's alias read empty on the swapd side). The credential, its fingerprint and `added` stay the occupant's. In phase 2 this means a deliberate re-import of a stale export moves hold/prefer state back to what the file says, the same as `--force` always did.
